...
export async function decrypt({config, password} = {}) {
  ...
  const {kek} = await _deriveKek({password, salt, version});

  // unwrap key derivation key
  const {wrappedKeySize} = VERSIONS.get(version);
  const unwrappedKey = await kek.unwrapKey({
    wrappedKey: await multihashDecode({
      expectedSize: wrappedKeySize, wrappedKey
    })
  });
  if(!unwrappedKey) {
    // invalid password
...
async function _createSecret({kdk, password, version} = {}) {
  ...
  return {
    secret: {
      version,
      salt: multihashEncode(algorithm.salt),
      wrappedKey: multihashEncode(wrappedKey)
    }
  };
}

async function _deriveKek({password, salt, version} = {}) {
  const {iterations} = VERSIONS.get(version);
  const {
    derivedBits: kekSecret,
    algorithm
  } = await deriveBits({bits: 256, iterations, password, salt});
...
async function _deriveKeys({kdk} = {}) {
  ...
  const hmacSecret = await kdk.sign({data: 'hmac'});
  const kakSecret = await kdk.sign({data: 'keyAgreementKey'});
