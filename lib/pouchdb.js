 ...
 * the uniqueness constraints, the document will be inserted. A `selector` may
 * be omitted if `doc._id` is set (and its value will be used to generate a
 * default `selector`).
 *
 * @param {object} options - The options to use.
 ...
 */
async function updateOne({doc, query, upsert = false, uniqueConstraints} = {}) {
  // get matching record
  ...
  if(!existing) {
    ...
    // attempt an insert
    return this.insertOne({doc, uniqueConstraints});
  }
