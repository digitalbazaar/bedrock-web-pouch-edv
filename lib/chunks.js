...
export async function initialize() {
  ...
  // `docId` and `chunkIndex` and serves as the primary unique index for
  // this collection
}
...
export async function upsert({edvId, docId, chunk} = {}) {
  ...
  const _id = _createId({localEdvId, docId, chunkIndex: chunk.index});
...
export async function get({edvId, docId, chunkIndex} = {}) {
  ...
  const {docs: [record]} = await _client.find({
    selector: {_id: _createId({localEdvId, docId, chunkIndex})},
    limit: 1
  });
  if(!record) {
...
function _createId({localEdvId, docId, chunkIndex}) {
  return `${localEdvId}:${docId}:${chunkIndex}`;
}
