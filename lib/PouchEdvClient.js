...
export class PouchEdvClient extends EdvClientCore {
  ...
  */
   ...
   */
  static async createEdv({config, password} = {}) {
    let edvClient;
    if(password !== undefined) {
      ...
      edvClient = new PouchEdvClient({
        ...
        keyResolver: _createKeyResolver({keyAgreementKey, edvConfig: config})
      });
    }
  ...
  static async fromLocalSecrets({edvId, password} = {}) {
    ...
    const edvConfigPromise = await edvs.get({id: edvId});
    ...
    return new PouchEdvClient({
      hmac, id: config.id, keyAgreementKey,
      keyResolver: _createKeyResolver({keyAgreementKey, edvConfig})
    });
  }
...
function _createKeyResolver({keyAgreementKey, edvConfig} = {}) {
  ...
  return async function keyResolver({id}) {
    if(id === keyAgreementKey.id) {
      const key = keyAgreementKey.export();
...
async function _lazyCreateSecret({id, password} = {}) {
